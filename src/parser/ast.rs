...
impl<'a, 'b> Node<'a, 'b> {
    pub fn name(&self) -> &str {
        match self {
            Self::T(Token { kind, loc: _, text: _ }) => &kind,
            Self::NT(s, _) => &s,
        }
    }
    pub fn string_rep(&self) -> String {
        match self {
            Self::T(Token { kind, loc: _, text: _ }) => kind.to_string(),
            Self::NT(s, c) => {
                let mut s = s.clone();
                s.push_str(" ::= ");
                if c.is_empty() {
...
pub trait NeededForAstNode : Clone {
    fn from_token(tok: &Token<&str>) -> Self;
}
...
pub struct FnExecutor<N: NeededForAstNode> {
    functions: HashMap<String, Box<dyn Fn(&Vec<String>, &Vec<N>) -> N>>
}

impl<N: NeededForAstNode> FnExecutor<N> {
    pub fn exec(&self, name: &Vec<String>, params: &Vec<N>) -> N {
        assert!(name.len() > 0);
        let fn_name = &name[0];
        let name_add = &name[1..];
...
impl<N: NeededForAstNode> AstResolver<N> {
    ...
    pub fn resolve(&self, node: &Node, params: &Vec<N>) -> N {
        match node {
            Node::NT(_, c) => {
    ...
    pub fn add_func<T>(&mut self, name: &str, f: T) where T: Fn(&Vec<String>, &Vec<N>) -> N + 'static {
        self.executor.functions.insert(name.to_string(), Box::new(f));
    }
}
